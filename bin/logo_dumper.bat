...
if not defined cs set cs=0
set /a cs=!cs!+1
echo.&echo         试图截取该图片!cs!.bmp
if "!offset!"=="0" (
...
rem 检测图像大小
if not defined cs set cs=1

if exist "!cs!.bmp" (
for %%a in ("!cs!.bmp") do set "bmp_size=%%~za"
...
for /f %%i in ('!xxd! -p -l 2 !cs!.bmp') do (
if not "%%i"=="424d" echo.&echo     #错误！!cs!.bmp这张图片并非是BMP格式！&pause&exit /b 1
if not "!blocksize!"=="!bmp_size!" echo.&echo     #警告！!cs!.bmp这张图片大小与源镜像的不符，可能无法显示！&echo.echo     #脚本默认忽略这个错误...&timeout /t 3 /nobreak >nul
)
!busybox! cat "!cs!.bmp" >> newlogo.img
set /a cs=cs+1
